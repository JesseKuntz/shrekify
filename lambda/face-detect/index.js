	...
	exports =
		{
			grayscale: function (canvas) {
				...
				var result = new Canvas.createCanvas(canvas.width, canvas.height);
				...
				while (pix > 0) {
					...
					output[(pix -= 4)] =
						output[(pix1 = pix + 1)] =
						output[(pix2 = pix + 2)] =
							input[pix] * 0.3 + input[pix1] * 0.59 + input[pix2] * 0.11;
				}
			...
			detect_objects: function () {
				...
				params.scale_upto = Math.floor(
					Math.log(
						Math.min(
							params.canvas.width / params.cascade.width,
							params.canvas.height / params.cascade.height
						)
					) / Math.log(params.scale)
				);
				var i;
				for (i = 0; i < params.cascade.stage_classifier.length; i++) {
					params.cascade.stage_classifier[i].orig_feature =
						params.cascade.stage_classifier[i].feature;
				}
				...
				function pre() {
					...
					for (i = 1; i <= interval; i++) {
						pyr[i * 4] = new Canvas.createCanvas(
					...
					for (i = next * 2; i < scale_upto + next * 2; i++) {
						pyr[i * 4 + 1] = new Canvas.createCanvas(
							Math.floor(pyr[i * 4 - next * 4].width / 2),
							Math.floor(pyr[i * 4 - next * 4].height / 2)
						);
						pyr[i * 4 + 1]
						...
						pyr[i * 4 + 2] = new Canvas.createCanvas(
							Math.floor(pyr[i * 4 - next * 4].width / 2),
							Math.floor(pyr[i * 4 - next * 4].height / 2)
						);
						pyr[i * 4 + 2]
						...
						pyr[i * 4 + 3] = new Canvas.createCanvas(
							Math.floor(pyr[i * 4 - next * 4].width / 2),
							Math.floor(pyr[i * 4 - next * 4].height / 2)
						);
						pyr[i * 4 + 3]
						...
						for (j = 0; j < cascade.stage_classifier.length; j++) {
							...
							for (k = 0; k < cascade.stage_classifier[j].count; k++) {
								...
								for (q = 0; q < orig_feature[k].size; q++) {
									feature[k].px[q] =
										orig_feature[k].px[q] * 4 + orig_feature[k].py[q] * step[orig_feature[k].pz[q]];
									feature[k].pz[q] = orig_feature[k].pz[q];
									feature[k].nx[q] =
										orig_feature[k].nx[q] * 4 + orig_feature[k].ny[q] * step[orig_feature[k].nz[q]];
							...
							for (y = 0; y < qh; y++) {
								for (x = 0; x < qw; x++) {
									...
									for (j = 0; j < cascade.stage_classifier.length; j++) {
										...
										var stageClassifierFeature = cascade.stage_classifier[j].feature;
										for (k = 0; k < cascade.stage_classifier[j].count; k++) {
											var feature_k = stageClassifierFeature[k];
											var p,
					...
					if (!(min_neighbors > 0)) return seq;
						...
						for (i = 0; i < seq2.length; i++) {
							...
							for (j = 0; j < seq2.length; j++) {
								...
								if (
									i != j &&
									seq2r1.x >= r2.x - distance &&
									seq2r1.y >= r2.y - distance &&
									seq2r1.x + seq2r1.width <= r2.x + r2.width + distance &&
									seq2r1.y + seq2r1.height <= r2.y + r2.height + distance &&
									(r2.neighbors > Math.max(3, seq2r1.neighbors) || seq2r1.neighbors < 3)
								) {
									flag = false;
									break;
								}
							...
							if (flag) result_seq.push(seq2r1);
						}
						return result_seq;
					}
				}

				return post([core(pre(1), 0, 0)]);
			}
		});
